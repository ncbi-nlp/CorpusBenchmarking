...
def _run_terminology_metric(metric_spec: MetricSpec, battery_config: BatteryConfig, corpora: dict[str, BenchmarkCorpus], contexts: dict[str, BenchmarkContext], workspace: GlobalWorkspace):
    results: list[Any] = []
    if not metric_spec.target_bundles:
        return results
...
def run_benchmark(battery_config: BatteryConfig) -> list[Any]:
    ...
    for metric_spec in battery_config.metrics:
        ...
        if metric_spec.metric_name in SUBSET_METRICS:
            results.extend(_run_subset_metric(metric_spec, battery_config, corpora, contexts))
        elif metric_spec.metric_name in CROSS_METRICS:
            results.extend(_run_cross_metric(metric_spec, battery_config, corpora, contexts))
        elif metric_spec.metric_name in TERMINOLOGY_METRICS:
            results.extend(_run_terminology_metric(metric_spec, battery_config, corpora, contexts, workspace))
        else:
            available_metrics = []
